...
func (a MyAdapter) toSql(datum types.Datum) (string, interface{}) {
	var sql string
	var value interface{}
	switch t := datum.DatumVariant.(type) {
...
// Build a sqlboiler query from the oso filter which is a representation of relational logic
func (a MyAdapter) BuildQuery(filter *types.Filter) (interface{}, error) {
	...
	for _, filter_relation := range filter.Relations {
		...
		// this can probably be looked up directly in sqlboiler some how
...
// execute the query returned by BuildQuery
...
func main() {
	...
	oso.RegisterClassWithNameAndFields(models.Repository{}, nil, "Repository", map[string]interface{}{
		"Name": "String",
		// could maybe also be autogenerated by sqlboiler
		"Org": types.Relation{
			Kind:       "one",
